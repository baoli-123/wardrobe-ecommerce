<template>
    <div>
      <el-dialog v-model="editDialogSon" @close="handleClose" title="编辑用户" width="400">
        <el-form :model="formData"
          ...
          <el-form-item  label="用户名" prop="userName">
            <el-input disabled="disabled"  v-model="formData.userName"></el-input>
          </el-form-item>
...
const submitForm = () => {
  ...
  ruleFormRef.value.validate((valid, fields) => {
    if (valid) {
      axios.post('/wardrobe_back/editUser',data).then(res => {
        alert(res.data)
